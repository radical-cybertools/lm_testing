...
'''
This script consumes a set of test cases (tc_*.json).  For each test case, it
will create a workload as described, and will prepare to run that workload via
...
The tasks are not actually executed - but a qsub script is created for each of
the above, and the workload execution is prepared up to the point where only the
node list is used to determine the actual node names to be used.
'''
...
if __name__ == '__main__':
  ...
  # visualizer = 'mute'
    ...
    launchers = ['fork', 'jsrun', 'orte', 'prrte']
    ...
    with open('summary.txt', 'w') as fout:
        ...
        # run test cases for all launchers
        for launcher in launchers:

            for tc in tcases:

              # print 'testing %s [%s]' % (tc['uid'], launcher)

                pwd = '%s_%s/' % (launcher, tc['uid'])
                os.mkdir(pwd)
